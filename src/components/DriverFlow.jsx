...
export default function DriverFlow({ API, token, me, year }) {
  ...
  const [busyDoc, setBusyDoc] = useState({}); // { [docCode]: true }
  const fileInputs = useRef({});

  // ---------- Bootstrap with smart step decision ----------
  useEffect(() => {
    bootstrap();
  ...
  const bootstrap = async () => {
    try {
      ...
      const personalIncomplete =
        !p || !p.first_name || !p.last_name || !p.address || !p.ssn_last4;
      ...
      const hasAnyExpense = (expenses || []).some((e) => e.amount != null);
      if (!hasAnyExpense) {
        setStep("business");
        return;
      }
  ...
  const pickFirm = async (id) => {
    await selectFirm(API, token, id);
    // после выбора фирмы ведём к заполнению профиля
    setStep("profile");
  };
  ...
  const saveProfile = async () => {
    ...
    // после сохранения идём к бизнесу
    await loadBusiness();
    setStep("business");
  };
    ...
    return (
      <div className="grid">
        <div className="card">
          ...
          <div className="note">Choose a firm to continue. You can change later.</div>
        ...
        <div className="card">
          ...
          <div className="kv" style={{ marginTop: 8 }}>
            ...
            <input
              ...
              onChange={(e) => setProfile((p) => ({ ...p, ssn_last4: e.target.value }))}
            />
          ...
          <div className="row" style={{ justifyContent: "space-between", marginTop: 12 }}>
            <button className="secondary" onClick={() => setStep("market")}>
              Back
            </button>
      ...
      <div className="grid">
        <div className="card">
          ...
          <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
            ...
            <button className="secondary" onClick={loadBusiness}>
              Refresh
            </button>
          </div>
  ...
  return (
    <div className="grid">
      ...
      <SubmitBar stage={status?.stage} canSubmit={canSubmit} onSubmit={submitAll} />
...
function Checklist({ title, list, busy, onPick, fileInputs }) {
  const groups = useMemo(() => {
    const personalCodes = new Set([
      "DL", "SSN Card", "US Passport", "Green Card", "Birth Certificate", "CONFIRM_PERSONAL"
    ]);
    const isPersonal = (d) => personalCodes.has(d) || String(d).includes("CONFIRM_PERSONAL");
    return {
    ...
    <div className="card">
      {stage === "collection" ? (
        <div className="row spread">
          ...
          <button disabled={!canSubmit} onClick={onSubmit}>Finish & submit</button>
        </div>
      ) : (
        <div className="row spread">
